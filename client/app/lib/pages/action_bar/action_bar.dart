...
class ActionBarState extends State<ActionBar> {
  ...
  final ContentGrpcClient contentClient = Get.find();
          ...
          Expanded(
            ...
            child: GestureDetector(
              onTap: () async {
                if (!await check()) return;
                if (action!.likeId == 0) {
                  final object = await actionClient.stub.like(
                    LikeReq(
                      type: ext.type,
                      refId: ext.refId,
                  ...
                  await actionClient.stub.delLike(
  ...
  Future<bool> check() async {
    ...
    if (action == null) {
      try {
        action = await actionClient.stub.getUserAction(
