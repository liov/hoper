...
class _RemoteBrowseViewState extends State<RemoteBrowseView> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final list = await _wire!.listFiles(_pathCtrl.text.trim());
      setState(() {
        _entries
          ..clear()
          ..addAll(list);
        _previewIndex = 0;
        _status = 'P2P · 房间 ${_roomCtrl.text.trim()} · ${list.length} 项';
      });
    } catch (e) {
          ...
          child: Column(
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: '直连 IP（可选）', border: OutlineInputBorder()),
              ),
        ...
        if (_entries.isNotEmpty)
          Expanded(
            ...
            child: PageView.builder(
              ...
              itemBuilder: (ctx, i) => _ThumbPreview(wire: _wire!, entry: _entries[i]),
            ),
          ),
        Expanded(
          flex: 3,
              ...
              return InkWell(
                ...
                child: DecoratedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      Expanded(child: _ThumbPreview(wire: _wire!, entry: e, maxEdge: 128, fit: BoxFit.cover)),
...
class _ThumbPreview extends StatelessWidget {
  const _ThumbPreview({required this.wire, required this.entry, this.maxEdge = 256, this.fit = BoxFit.contain});
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: wire.fetchThumb(entry.id.isNotEmpty ? entry.id : entry.name, maxEdge: maxEdge),
      builder: (ctx, snap) {
