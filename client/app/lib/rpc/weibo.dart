...
class WeiboClient {
  ...
  Future<WeiboList?> getList({
    ...
    globalService.logger.fine(
      "uid:$uid,page:$page,feature:$feature,sinceId:$sinceId",
    );
    ...
    return response.getWeiboData(
      (v) => WeiboList.fromJson(v as Map<String, dynamic>),
    );
  }

  Future<WeiboOriginList?> getOriginalList({
    required int uid,
    required int page,
    required int feature,
    String? sinceId,
    ...
    final response = await httpClient.get(
      '/searchProfile',
      queryParameters: {'uid': uid, 'page': page, 'hasori': 1},
    );
    return response.getWeiboData(
      (v) => WeiboOriginList.fromJson(v as Map<String, dynamic>),
    );
  }
}
