...
export async function  request<D,T extends UniNamespace.RequestOptions = UniNamespace.RequestOptions>(options:T):Promise<D>{
...
export async function  get<D,T extends UniNamespace.RequestOptions = UniNamespace.RequestOptions>(options:T):Promise<D>{
    return new Promise(function (resolve, reject) {
        uni.request({...options,
            success : (res)=>{
                resolve(res.data as D);
...
export async function  post<D,T extends UniNamespace.RequestOptions = UniNamespace.RequestOptions>(options:T):Promise<D>{
    return new Promise(function (resolve, reject) {
        uni.request({...options,
            success : (res)=>{
                resolve(res.data as D);
...
interface UniRespone<T extends string | AnyObject | ArrayBuffer> extends UniApp.RequestSuccessCallbackResult{
    data:T
}
...
class UniRequest{
    ...
    public request<T extends string | AnyObject | ArrayBuffer = any, D extends string | AnyObject | ArrayBuffer | undefined = any>(method: 'GET'|'POST', url:string, data?:D, config?:UniRequestConfig):Promise<UniRespone<T>>{
        return new Promise((resolve, reject) => {
            url = (config?.baseUrl || this.defaults.baseUrl)+ url;
            let header = config?.header ? Object.assign(this.defaults.header, config.header) : this.defaults.header;
            let timeout = config?.timeout ? config.timeout : this.defaults.timeout;
    ...
    // 发起get请求
    public get<T extends string | AnyObject | ArrayBuffer = any, D extends string | AnyObject | ArrayBuffer | undefined = any>(url:string, params?:D, config?:UniRequestConfig){
        return this.request<T, D>('GET', url, params, config)
    }
    // 发起post请求
    public post<T extends string | AnyObject | ArrayBuffer = any, D extends string | AnyObject | ArrayBuffer | undefined = any>(url:string, data?:D, config?:UniRequestConfig){
