...
const actions = {
    ...
    async login(params) {
        try {
            const {statusCode,
                data: {details},
            } = await uniHttp.post("/api/v1/user/login",
                params
            );
    ...
    async signup(params) {
        try {
            const {
                data: {details},
            } = await uniHttp.post("/api/v2/user",
                params,
                {
                header:{
    ...
    async appendUsersById(ids: number[]) {
        ...
        if (noExistsId.length > 0) {
            ...
            if (data.code && data.code !== 0)  await uni.showToast({title:data.message, icon:"error",duration: 1000});
