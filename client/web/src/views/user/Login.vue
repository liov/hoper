    ...
    <van-row type="flex" justify="center">
      <van-col span="18">
        ...
        <van-form @submit="onSubmit">
          <van-field v-if="type" v-model="username" name="name" label="用户名" placeholder="用户名"
            :rules="[{ required: true, message: '请填写用户名' }]" />
          <van-field v-if="type" name="gender" label="性别" :rules="[{ required: true, message: '请填选择性别' }]">
            <template #input>
          ...
          <van-field v-if="type" v-model="phone" name="phone" label="手机" placeholder="手机" :rules="[
            {
              required: true,
          ...
          <van-field v-if="type" v-model="mail" name="mail" label="邮箱" placeholder="邮箱" :rules="[
            {
              required: true,
          ...
          <van-field v-if="!type" v-model="account" name="input" label="账号" placeholder="邮箱或手机或hoper号"
            :rules="[{ required: true, message: '请填写邮箱或手机或hoper号' }]" />
          <van-field v-model="password" type="password" name="password" label="密码" placeholder="密码"
            :rules="[{ required: true, message: '请填写密码' }]" />
          <van-field v-if="type" type="password" name="password_confirm" v-model="password_confirm" label="密码确认"
