...
Two services:
- `rb/` — video/image processing (ffmpeg, axum, tokio); exposes staticlib + cdylib for FFI
- `message/` — WebSocket messaging (axum-ws, dashmap)

```bash
cd server/rust/rb && cargo build --release
# 远程相册：改 server/rust/rb、proto/remotebrowse、client/app/lib/remotebrowse 后
# Agent 必须执行（见 .cursor/rules/remotebrowse-pack.mdc），禁止只提示用户编译：
cd server/rust/rb && bash scripts/macos/build.sh
