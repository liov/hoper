...
func FindFile2(path string, deep int8, num int) ([]string, error) {
	...
	var open = atomic.NewInt32(0)
	var goClose = atomic.NewInt32(0)
	open.Add(1)
	go subDirFiles2(wd, path, file, deep, 0, ctx, open, goClose)
	open.Add(1)
	go supDirFiles2(wd+"/", path, file, deep, 0, ctx, open, goClose)
	var files []string
...
func subDirFiles2(dir, path string, file chan string, deep, step int8, ctx context.Context, open, goClose *atomic.Int32) {
	defer func() {
		goClose.Add(1)
		if goClose.Load() == open.Load() {
			close(file)
		}
	}()
	step += 1
	...
	for i := range fileInfos {
		if fileInfos[i].IsDir() {
			...
			open.Add(1)
			go subDirFiles2(filepath.Join(dir, fileInfos[i].Name()), path, file, deep, step, ctx, open, goClose)
		}
	}
}

func supDirFiles2(dir, path string, file chan string, deep, step int8, ctx context.Context, open, goClose *atomic.Int32) {
	defer func() {
		goClose.Add(1)
		if goClose.Load() == open.Load() {
			close(file)
	...
	for i := range fileInfos {
		if fileInfos[i].IsDir() {
			...
			open.Add(1)
			go subDirFiles2(filepath.Join(dir, fileInfos[i].Name()), path, file, deep, 0, ctx, open, goClose)
		}
	}
	open.Add(1)
	go supDirFiles2(dir, path, file, deep, step, ctx, open, goClose)
}
