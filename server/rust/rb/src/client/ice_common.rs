...
pub async fn run_ice(
    ...
    tracing::debug!(controlling, timeout_ms = timeout.as_millis(), "ice run start");
    ...
    tracing::debug!(controlling, "ice remote credentials ok");
    agent
    ...
    if controlling {
        let conn = tokio::time::timeout(timeout, agent.dial(cancel_rx, remote_ufrag, remote_pwd))
            .await
            .map_err(|_| "ice dial timeout".to_string())?
            .map_err(|e| e.to_string())?;
        tracing::info!(controlling, "ice dial ok");
    ...
    let conn = tokio::time::timeout(timeout, agent.accept(cancel_rx, remote_ufrag, remote_pwd))
        .await
        .map_err(|_| "ice accept timeout".to_string())?
        .map_err(|e| e.to_string())?;
    tracing::info!(controlling, "ice accept ok");
    let _ = cancel_tx.send(()).await;
...
async fn build_agent(ufrag: &str, pwd: &str, controlling: bool) -> Result<Agent, String> {
    ...
    // 仅 UDP4 + 关 mDNS：避免 macOS 上对 fe80 网卡 bind 失败、无 IPv6 时 STUN 刷屏（见 agent_gather WARN）。
    let cfg = AgentConfig {
        urls,
