...
func auth(ctx context.Context, update bool) (*model.AuthInfo, error) {
	...
	if ok {
		cache := cacheTmp.(*jwt2.Claims[*model.AuthInfo])
	...
	authorization, err := jwt2.Auth[*model.AuthInfo](ctx, global.Conf.User.TokenSecretBytes)
	if err != nil {
		return nil, user.UserErrNoLogin
	}
	...
	if metadata.Data == nil {
		metadata.Data = &model.ClientInfo{
			Auth: authorization.Auth,
		}
	} else {
		metadata.Data.(*model.ClientInfo).Auth = authorization.Auth
	}
...
func Device(ctx context.Context) *contextx.DeviceInfo {
	...
	if metadata.Data != nil {
		clientInfo := metadata.Data.(*model.ClientInfo)
