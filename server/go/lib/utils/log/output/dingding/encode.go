...
func getJSONEncoder() *dingEncoder {
	return _jsonPool.Get().(*dingEncoder)
}

func putJSONEncoder(enc *dingEncoder) {
...
func newJSONEncoder(cfg *zapcore.EncoderConfig, spaced bool) *dingEncoder {
	...
	if cfg.NewReflectedEncoder == nil {
		cfg.NewReflectedEncoder = encode.DefaultReflectedEncoder
	}
...
func (enc *dingEncoder) addElementSeparator() {
	last := enc.buf.Len() - 1
	if last < 0 {
		return
	}
