...
func (c *RequestContext[REQ, P]) Error(args ...interface{}) {
	args = append(args, zap.String(log.TraceId, c.TraceID))
	log.Default.Error(args...)
}

func (c *RequestContext[REQ, P]) HandleError(err error) {
	if err != nil {
		log.Default.Error(err.Error(), zap.String(log.TraceId, c.TraceID))
	}
}

func (c *RequestContext[REQ, P]) ErrorLog(err, originErr error, funcName string) error {
