...
import (
	...
	cuzproto "github.com/liov/hoper/v2/protobuf/utils/proto/gogo"
)
		...
		if m := file.GetMessage(typeWithoutPrefix); m != nil {
			...
			if p.IsAny(typeName) {
				p.scalars[typeName] = &Type{ModelDescriptor: ModelDescriptor{
					PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", //TODO generate gqlgen.yml
					TypeName:   ScalarAny,
				}}
				return nil
			}
		...
		if message := p.getMessageType(file, typeName); message != nil {
			...
			for _, field := range message.GetField() {
				...
				if gogoproto.IsCastType(field) {
					...
					switch typ {
					case "int8":
						p.scalars[ScalarInt8] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarInt8}}
					case "uint8":
						p.scalars[ScalarUint8] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarUint8}}
					}
				} else {
					switch *field.Type {
					case descriptor.FieldDescriptorProto_TYPE_BYTES:
						p.scalars[ScalarBytes] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarBytes}}

					case descriptor.FieldDescriptorProto_TYPE_FLOAT:
						p.scalars[ScalarFloat32] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarFloat32}}

					case descriptor.FieldDescriptorProto_TYPE_INT64,
						descriptor.FieldDescriptorProto_TYPE_SINT64,
						descriptor.FieldDescriptorProto_TYPE_SFIXED64:
						p.scalars[ScalarInt64] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarInt64}}

					case descriptor.FieldDescriptorProto_TYPE_INT32,
						descriptor.FieldDescriptorProto_TYPE_SINT32,
						descriptor.FieldDescriptorProto_TYPE_SFIXED32:
						p.scalars[ScalarInt32] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarInt32}}

					case descriptor.FieldDescriptorProto_TYPE_UINT32,
						descriptor.FieldDescriptorProto_TYPE_FIXED32:
						p.scalars[ScalarUint32] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarUint32}}

					case descriptor.FieldDescriptorProto_TYPE_UINT64,
						descriptor.FieldDescriptorProto_TYPE_FIXED64:
						p.scalars[ScalarUint64] = &Type{ModelDescriptor: ModelDescriptor{PackageDir: "github.com/liov/hoper/v2/utils/net/http/api/graphql", TypeName: ScalarUint64}}
					}
				}
...
func (p *Plugin) getMethodType(rpc *descriptor.MethodDescriptorProto) cuzproto.Operation {
	if rpc.Options != nil {
		v, err := proto.GetExtension(rpc.Options, cuzproto.E_GraphqlOperation)
		if err == nil {
			tt := v.(*cuzproto.Operation)
			if tt != nil {
				return *tt
			}
		}
	...
	return cuzproto.Operation_Default
}

func (p *Plugin) getServiceType(svc *descriptor.ServiceDescriptorProto) cuzproto.Operation {
	if svc.Options != nil {
		v, err := proto.GetExtension(svc.Options, cuzproto.E_Operation)
		if err == nil {
			tt := v.(*cuzproto.Operation)
			if tt != nil {
				return *tt
...
func (p *Plugin) GraphQLType(field *descriptor.FieldDescriptorProto, messagesIn map[string]*Type) string {
	...
	if resolveRequired(field) || *field.Type != 11 {
		suffix += "!"
	}
	...
	for svci, svc := range file.GetService() {
		for rpci, rpc := range svc.GetMethod() {
			...
			if rpc.GetServerStreaming() {
				...
				case cuzproto.Operation_Default:
					switch ttt := p.getServiceType(svc); ttt {
					case cuzproto.Operation_Default, cuzproto.Operation_Mutation:
						p.mutations = append(p.mutations, m)
					case cuzproto.Operation_Query:
						p.queries = append(p.queries, m)
					}
				case cuzproto.Operation_Mutation:
					p.mutations = append(p.mutations, m)
				case cuzproto.Operation_Query:
					p.queries = append(p.queries, m)
				}
