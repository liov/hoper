...
func (*UserDao) ExitByEmailORPhone(ctx *koko.Ctx,db *gorm.DB, mail, phone string) (bool, error) {
	DBNotNil(&db)
	var err error
	var count int64
	if mail != "" {
...
func (d *UserDao) GetByEmailORPhone(ctx *koko.Ctx,db *gorm.DB, email, phone string, fields ...string) (*model.User, error) {
	DBNotNil(&db)
	var user model.User
	var err error
	if len(fields) > 0 {
...
func (*UserDao) Creat(ctx *koko.Ctx,db *gorm.DB, user *model.User) error {
	DBNotNil(&db)
...
func (*UserDao) GetByPrimaryKey(ctx *koko.Ctx,db *gorm.DB, id uint64) (*model.User, error) {
	DBNotNil(&db)
	var user model.User
...
func (*UserDao) SaveResumes(ctx *koko.Ctx,db *gorm.DB, userId uint64, resumes []*model.Resume, originalIds []uint64, device *model.UserDeviceInfo) error {
	DBNotNil(&db)
	if len(resumes) == 0 {
		return nil
	}
...
func (*UserDao) ActionLog(ctx *koko.Ctx,db *gorm.DB, log *model.UserActionLog) error {
...
func (*UserDao) ResumesIds(ctx *koko.Ctx,db *gorm.DB, userId uint64) ([]uint64, error) {
	DBNotNil(&db)
