...
import (
	...
	. "github.com/liov/hoper/go/v2/tools/graphql/plugin"
	"gopkg.in/yaml.v2"
)
...
func (p *plugin) Name() string                                   { return "gqlcfg" }
...
func (p *plugin) Generate(file *generator.FileDescriptor) {
	for _, fileName := range p.Request.FileToGenerate {
		if fileName == file.GetName() {
			...
			cfg := &config{
				SchemaFilename: []string{"./*.graphqls"},
				Exec:           packageConfig{Filename: "generated.gql.go"},
...
type (
	config struct {
		...
		Federation     packageConfig           `yaml:"federation"`
