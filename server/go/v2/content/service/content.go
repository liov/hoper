...
func (*ContentService) CreateFav(ctx context.Context, req *content.CreateFavReq) (*empty.Empty, error) {
	ctxi, span := user.CtxFromContext(ctx).StartSpan("")
	defer span.End()
...
func (*ContentService) EditFav(ctx context.Context, req *content.EditFavReq) (*empty.Empty, error) {
	ctxi, span := user.CtxFromContext(ctx).StartSpan("")
	defer span.End()
	auth, err := ctxi.GetAuthInfo(AuthWithUpdate)
	if err != nil {
