...
[go-how-is-two-dimensional-arrays-memory-representation](go-how-is-two-dimensional-arrays-memory-representation)

[what-is-a-concise-way-to-create-a-2d-slice-in-go](what-is-a-concise-way-to-create-a-2d-slice-in-go)
### 15. 访问 map 中不存在的 key
如果访问了 map 中不存在的 keyGo 则会返回元素对应数据类型的零值，比如 nil、'' 、false 和 0，取值操作总有值返回，故不能通过取出来的值来判断 key 是不是在 map 中。
...
```
如果需要使用 for range 迭代访问字符串中的字符（unicode code point / rune），标准库中有 "unicode/utf8" 包来做 UTF8 的相关解码编码。另外[utf8string]( utf8string) 也有像 func (s *String) At(i int) rune 等很方便的库函数。
### 19. 字符串并不都是 UTF8 文本
...
```
如果你去 [Go Playground](Go Playground) 重复运行上边的代码，输出是不会变的，只有你更新代码它才会重新编译。重新编译后迭代顺序是被打乱的：
...
误区：GOMAXPROCS 表示执行 goroutine 的 CPU 核心数，[参考文档](参考文档)
