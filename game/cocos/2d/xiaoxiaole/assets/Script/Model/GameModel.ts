...
export default class GameModel {
   ...
   * @returns {([]|string|*)[]}
   */
  checkPoint(x:number, y:number, recursive=false):[Vec2[],string,number,Vec2]{
    ...
    let result:[Vec2[],string,number,Vec2] = [samePoints, newCellStatus, this.cells[y][x].type, v2(x, y)];
  ...
  checkWithDirection(x:number, y:number, direction) {
    let queue = [];
    let vis = [];
    vis[x + y * 9] = true;
    queue.push(v2(x, y));
  ...
  // controller调用的主要入口
  // 点击某个格子
  selectCell(pos:Vec2): [CellModel[],any[]] {
  ...
  //生成新cell
  createNewCell(pos:Vec2, status:string, type:number) {
    if (status == "") {
      return;
    }
  ...
  processBomb(bombModels, cycleCount) {
    while (bombModels.length > 0) {
      ...
      bombModels.forEach(function (model:CellModel) {
